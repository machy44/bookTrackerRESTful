...
bookShelvesRouter.route('/')
  .get((req, res) => {
      ...
      .then(book=>{ if(!book) return res.status(404).json({ msg: "book id does not exist" });
            ...
            sequelize.query(queryValue, { replacements: { bookId: req.params.bookId }, type: sequelize.QueryTypes.SELECT })
            .then(shelves => {
              if (!shelves.length) return res.status(200).json( shelves ); // return empty array
              res.status(200).json(collectionJSON(req.headers.host, '/api/shelves',  shelves , { query: false, template: false }));
            })
